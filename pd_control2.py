...
def dydt(t, y, q_ddot):
        q = y[:4]     
        q_dot = y[4:] 
        dydt = np.concatenate([q_dot, q_ddot.flatten()])  
        return dydt

y0 = np.concatenate([q_vals.flatten(), q_dot_vals.flatten()])
...
for i in range(steps-2):
    ...
    tspan = (i * dt, (i + 1) * dt)
    sol = solve_ivp(lambda t, y: dydt(t, y, q_ddot), tspan, y0, method='RK45')
    y= sol.y[:, -1]
    q_vals=y[:4].reshape(4,1)
    q_dot_vals[4:]
    fourR_planar.plot(q_vals.reshape(-1,1).flatten())
    y0=np.concatenate([q_vals.flatten(), q_dot_vals.flatten()])

print(f"EE_position:{ee_positions}")
